...
# List of user tokens for multiple accounts
TOKENS = [
    "YOUR_TOKEN_1",
    "YOUR_TOKEN_2",  # Add more tokens for multiple accounts
]
...
async def create_client(token):
        ...
        if message.author.id == TARGET_USER_ID and message.channel.id in MONITOR_CHANNEL_IDS:
            ...
            if notification_channel:  # Ensure the notification channel exists
                try:
                    ...
                    print(f"Message forwarded successfully with Rain role ping from {client.user}!")
                    ...
                    for component in message.components:
                        for button in component.children:
                            if button.emoji.name == "💵":
                                await button.click()
                                print(f"💵 button clicked by {client.user}!")
                                break
    ...
    try:
        ...
        print(f"Invalid token for account {token}. Please check your user token.")
    except Exception as e:
        print(f"An unexpected error occurred for account {token}: {e}")

# Run the self-bots for multiple accounts concurrently
async def run_multiple_bots():
    tasks = []
    for token in TOKENS:
        tasks.append(create_client(token))  # Create a bot client for each token
